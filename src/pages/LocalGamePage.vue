...
import type { Action, MoveAction, Owner, Piece } from '@/types/game';
...
const humanTurn = computed(
  () =>
    ...
    state.value.currentPlayerId !== state.value.aiPlayerId,
);
  ...
  const player =
    state.value.mode === 'pvp'
      ...
      : id === state.value.aiPlayerId
        ? 'AI'
        : '你';
  const ownerText = owner == null ? '未确定' : owner === 1 ? '红方' : '蓝方';
  return `${player}（${ownerText}）行棋`;
});

const actionHint = computed(() =>
  state.value.playerOwners[state.value.currentPlayerId] == null
    ? `阵营未确定：翻开一张牌（${catOnly ? '只有猫能吃鼠' : '除象外都能吃鼠'}）`
...
const moveAt = (r: number, c: number): MoveAction | undefined =>
  state.value.validMoves.find((move) => move.to.r === r && move.to.c === c);

// 只有已翻开且属于当前阵营的棋子可以被选中，并据此生成移动提示。
const select = (r: number, c: number): void => {
  if (!humanTurn.value || animating.value || aiThinking.value) return;

  const piece = getPieceAt(state.value.board, { r, c });
  const owner = state.value.playerOwners[state.value.currentPlayerId];
  if (!piece || !piece.revealed || owner == null || piece.owner !== owner) return;
...
const clickCell = (r: number, c: number): void => {
  if (!humanTurn.value || animating.value || state.value.gameOver) return;
...
const execute = (action: Action): void => {
  ...
  const previousTurn = game.turnCount;
  const previousLastRevealedOwner = game.lastRevealedOwner;
  ...
  game.history.push({
    action,
    revertCtx: context,
    prevPlayerId: previousPlayerId,
    prevPlayerOwners: previous,
    prevTurn: previousTurn,
    captured,
    prevLastRevealedOwner: previousLastRevealedOwner,
  });

  animating.value = true;
  window.setTimeout(() => {
    if (version !== gameVersion.value || game !== state.value) return;

    animating.value = false;
    if (
      !state.value.gameOver &&
      state.value.mode === 'pve' &&
      state.value.currentPlayerId === state.value.aiPlayerId
    ) {
      triggerAI();
    }
...
const triggerAI = async (): Promise<void> => {
  ...
  const action = await findBestMove(game);
  if (version !== gameVersion.value || game !== state.value) return;
...
const restart = (): void => {
  ...
  animating.value = false;
  aiThinking.value = false;
  state.value = createState(mode, 1, difficulty, catOnly);
};

const alive = (owner: Owner): number => countAlive(state.value.board, owner);
</script>
...
<template>
  <main class="game-screen">
    ...
    <aside class="panel" :class="{ 'is-collapsed': panelCollapsed }">
      ...
      <div class="captured-section">
        <div
          v-for="owner in [2, 1] as Owner[]"
          :key="owner"
          class="captured-block"
        >
          ...
          <div class="captured-list">
            ...
            >
              <img
                :src="classicPieceImage(piece.type)"
                :alt="PIECE_TYPES[piece.type].char"
              >
            </span>
          </div>
      ...
      <div class="btn-group">
        ...
        <button class="btn" @click="router.push('/')">返回菜单</button>
      </div>
    </aside>
  </main>
  ...
  <div v-if="state.gameOver" class="overlay">
    <div class="card over-card">
      <h2
        class="winner-text"
        :class="state.winner === 1 ? 'red' : 'black'"
      >
